...
def get_contours(file_name, approx_convex):
	...
	count = np.bincount(grey.flatten())

	main_colors = (-count).argsort()[:numcolors]

	res = []
	colors = [float]*numcolors
	for i in range(numcolors):
		mask = np.array(grey)
		mask[np.abs(grey-main_colors[i])> 5]= 255
		mask[mask < 255] = 0
		mask = cv2.bilateralFilter(mask,9,75,75)
		# ret,mask = cv2.threshold(mask,127,255,cv2.THRESH_BINARY)
		cv2.imwrite(str(i) + '_grey.jpg', mask)
		img = orig[::]
		cnts, h = cv2.findContours(mask, cv2.RETR_LIST,cv2.CHAIN_APPROX_NONE)
		print len(cnts)
		cv2.drawContours(img, cnts, -1,(0,255,0),3)
		cv2.imwrite(str(i) + '_mask.jpg', img)
